function [ff,xx] = BF_JitteredParallelScatter(dataCell,names,addMeans,doveTail,makeFigure,extraParams)
...
if doveTail
    ...
    for i = 1:numGroups
        ...
        if strfind(names{i},'OSLOM')
            color = theColors{1};
        end
        if strfind(names{i},'Clique')
            color = theColors{2};
        end
        if strfind(names{i},'NNMF')
            color = theColors{3};
        end
        if strfind(names{i},'Infomap')
            color = theColors{4};
        end
        if strfind(names{i},'SLPA')
            color = theColors{5};
        end
           
        
        plot(customOffset+i+f*offsetRange/2,x,'-','color',color,'LineWidth',2)
        plot(customOffset+i-f*offsetRange/2,x,'-','color',color,'LineWidth',2)
        plot(customOffset+i+[-f(1),+f(1)]*offsetRange/2,min(x)*ones(2,1),'-','color',color,'LineWidth',0.1)
        plot(customOffset+i+[-f(end),f(end)]*offsetRange/2,max(x)*ones(2,1),'-','color',color,'LineWidth',0.1)     
        % Plot top and bottom
...
if ~isempty(customSpot)
    for i = 1:numGroups
        ...
        plot(customOffset + i + xRand,dataCell{i},customSpot,'color',color)
...
for i = 1:numGroups
    ...
    try
        brightColor = brighten('k',brightenAmount);
    catch
        brightColor = 'k';
    end  
